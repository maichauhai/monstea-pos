...
// DEFAULT DATA & MENU IMAGES — defined in data.js (loaded before app.js)
...
function renderInventory(){
    ...
    if(ntbEl){
      if(!needList.length){ntbEl.innerHTML='';}
        ...
        let ntbHtml=`<div style="background:rgba(255,107,107,0.08);border:1px solid rgba(255,107,107,0.25);border-radius:var(--radius-md);padding:14px;">`;
        ntbHtml+=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span style="font-weight:700;font-size:0.85rem;">🛒 Gợi ý nhập hàng (${needList.length})</span><button class="btn btn-secondary btn-sm" onclick="copyNeedToBuyList()" style="font-size:0.68rem;padding:3px 8px;">📋 Copy</button></div>`;
        groupKeys.forEach(sup=>{
          const items=groups[sup];
          const supColor=sup==='Chưa gán NCC'?'var(--text-muted)':'var(--accent-blue)';
          ntbHtml+=`<div style="margin-bottom:8px;"><div style="font-size:0.75rem;font-weight:700;color:${supColor};margin-bottom:4px;padding:2px 6px;background:rgba(96,165,250,0.08);border-radius:4px;display:inline-block;">📦 ${esc(sup)} (${items.length})</div>`;
          ntbHtml+=`<div style="display:flex;flex-direction:column;gap:4px;">`;
          items.forEach(i=>{
            ...
            ntbHtml+=`<div style="display:flex;align-items:center;gap:6px;padding:4px 8px;border-radius:8px;background:${i.s.status==='danger'?'rgba(255,107,107,0.12)':'rgba(245,158,11,0.08)'}">`;
            ntbHtml+=`<span style="font-size:0.82rem;">${sI[i.s.status]}</span>`;
            ntbHtml+=`<span style="flex:1;font-size:0.78rem;font-weight:600;">${esc(i.name)} ${linkHtml}</span>`;
            ntbHtml+=`<span style="font-size:0.7rem;color:var(--text-muted);">${i.s.stock} ${i.unit}${i.s.daysLeft<999?' (≈'+i.s.daysLeft+'d)':''}</span>`;
            ntbHtml+=need>0?`<span style="font-size:0.7rem;color:var(--accent-green);font-weight:600;">→ ${packText}</span>`:'';
            ntbHtml+=`</div>`;
          });
          ntbHtml+=`</div></div>`;
        });
        ntbHtml+=`</div>`;
        ntbEl.innerHTML=ntbHtml;
      }
    }
    // Table with SLN
...
<input type="text" id="editIngSupplier" value="${esc(i.supplier||'')}" placeholder="VD: Shopee, Chợ Thủ Đức, Metro...">
