...
function editStaff(id){const s=state.staff.find(x=>String(x.id)===String(id));if(!s)return;
const body=`<div style="display:flex;flex-direction:column;gap:12px;">
...
function saveEditStaff(id){const s=state.staff.find(x=>String(x.id)===String(id));if(!s)return;
...
const dup=activeItems(state.staff).find(x=>String(x.id)!==String(id)&&x.password===p);
...
function deleteStaff(id){const s=state.staff.find(x=>String(x.id)===String(id));if(s){s._deleted=true;s._lastModified=Date.now();}saveState();renderSettings();renderAttendance();toast('🗑️ Đã xóa');}
