...
// CHECKLIST
// ═══════════════════════════════════════
const PREPARED_TOPPING_DAY_MS=24*60*60*1000;
function currentActorName(){return currentStaffName||(currentRole==='owner'?'Chủ quán':'?');}
function preparedToppingDateKey(ts){const d=new Date(Number(ts)||Date.now());return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;}
function preparedToppingTimeKey(ts){const d=new Date(Number(ts)||Date.now());return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;}
function preparedToppingDateTimeInput(ts){const d=new Date(Number(ts)||Date.now());return `${preparedToppingDateKey(d.getTime())}T${preparedToppingTimeKey(d.getTime())}`;}
function preparedToppingDateTimeLabel(ts){const d=new Date(Number(ts)||Date.now());return d.toLocaleString('vi-VN',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});}
function preparedDurationText(ms){const totalMinutes=Math.max(0,Math.floor(Math.abs(ms)/60000));if(totalMinutes<60)return 'dưới 1 giờ';const totalHours=Math.floor(totalMinutes/60),days=Math.floor(totalHours/24),hours=totalHours%24;return days?`${days} ngày ${hours} giờ`:`${hours} giờ`;}
function preparedToppingStatus(item){const remain=Number(item.expireAt||0)-Date.now();if(remain<=0)return{cls:'expired',label:`Quá hạn ${preparedDurationText(remain)}`};if(remain<=PREPARED_TOPPING_DAY_MS)return{cls:'warn',label:`Còn ${preparedDurationText(remain)}`};return{cls:'ok',label:`Còn ${preparedDurationText(remain)}`};}
function preparedToppingSnapshotText(item){return `${item.name||'?'} | ${item.staffName||'?'} | ${preparedToppingDateTimeLabel(item.madeAt)} | hạn ${Number(item.shelfLifeDays)||7} ngày${item.note?' | '+item.note:''}`;}
function preparedToppingStaffOptions(selectedId){const staff=activeItems(state.staff||[]),selected=String(selectedId||currentStaffId||'');const opts=staff.map(s=>`<option value="${s.id}" ${String(s.id)===selected?'selected':''}>${esc(s.name)}</option>`).join('');return opts||`<option value="">${esc(currentActorName())}</option>`;}
function selectedPreparedToppingStaff(selectId){const id=Number(document.getElementById(selectId)?.value);const staff=(state.staff||[]).find(s=>Number(s.id)===id);if(staff)return{id:staff.id,name:staff.name};return{id:currentStaffId||null,name:currentActorName()};}
function preparedToppingsSorted(includeDeleted){normalizePreparedToppingsFor(state);return (state.preparedToppings||[]).filter(t=>includeDeleted||!isDeleted(t)).sort((a,b)=>(Number(b.madeAt)||itemStamp(b))-(Number(a.madeAt)||itemStamp(a)));}
function appendPreparedToppingLog(item,action,before,after){const stamp=Date.now(),actor=currentActorName();const entry={syncId:makeSyncId('topping-log'),_lastModified:stamp,action,time:`${today()} ${nowTime()}`,actorStaffId:currentStaffId||null,actorName:actor,before,after};item.history=normalizeArrayLike(item.history);item.history.push(entry);if(!state.editLog)state.editLog=[];state.editLog.push({syncId:makeSyncId('edit'),_lastModified:stamp,action:`TOPPING ${action}`,time:entry.time,staffName:actor,itemRef:itemRef(item),invoiceRef:`topping:${itemRef(item)}`,before,after});}
function renderPreparedToppings(){const listEl=document.getElementById('preparedToppingList'),summaryEl=document.getElementById('preparedToppingSummary'),staffEl=document.getElementById('preparedToppingStaff');if(!listEl)return;normalizePreparedToppingsFor(state);if(staffEl)staffEl.innerHTML=preparedToppingStaffOptions(currentStaffId);const items=preparedToppingsSorted(false);const expired=items.filter(i=>preparedToppingStatus(i).cls==='expired').length,warn=items.filter(i=>preparedToppingStatus(i).cls==='warn').length;if(summaryEl)summaryEl.textContent=items.length?`${items.length} topping đang theo dõi · ${expired} quá hạn · ${warn} sắp hết hạn`:'Mặc định hạn dùng 7 ngày';listEl.innerHTML=items.map(item=>{const st=preparedToppingStatus(item),ref=jsString(itemRef(item));return `<div class="prepared-topping-row"><div class="prepared-topping-main"><div class="prepared-topping-name">${esc(item.name)}</div><div class="prepared-topping-meta">${esc(item.staffName||'?')} nấu ${preparedToppingDateTimeLabel(item.madeAt)} · hạn ${Number(item.shelfLifeDays)||7} ngày</div>${item.note?`<div class="prepared-topping-note">${esc(item.note)}</div>`:''}</div><div class="prepared-topping-age">Đã ${preparedDurationText(Date.now()-Number(item.madeAt||Date.now()))}</div><div><span class="prepared-topping-badge ${st.cls}">${esc(st.label)}</span></div><div class="prepared-topping-actions"><button class="btn btn-secondary btn-sm" onclick="showPreparedToppingEditor('${ref}')">Sửa</button><button class="btn btn-danger btn-sm" onclick="deletePreparedTopping('${ref}')">Xóa</button></div></div>`;}).join('')||'<div class="prepared-topping-empty">Chưa có topping nào được ghi hôm nay</div>';}
function addPreparedTopping(){normalizePreparedToppingsFor(state);const name=document.getElementById('preparedToppingName')?.value.trim(),days=Number(document.getElementById('preparedToppingDays')?.value)||7,note=document.getElementById('preparedToppingNote')?.value.trim()||'';if(!name){toast('Nhập tên topping đã nấu');return;}const staff=selectedPreparedToppingStaff('preparedToppingStaff'),madeAt=Date.now();const item={id:state.nextPreparedToppingId++,syncId:makeSyncId('prep-topping'),_lastModified:madeAt,name,staffId:staff.id,staffName:staff.name,madeAt,madeDate:preparedToppingDateKey(madeAt),madeTime:preparedToppingTimeKey(madeAt),shelfLifeDays:days,expireAt:madeAt+days*PREPARED_TOPPING_DAY_MS,status:'active',note,history:[]};appendPreparedToppingLog(item,'THÊM','',preparedToppingSnapshotText(item));state.preparedToppings.push(item);document.getElementById('preparedToppingName').value='';document.getElementById('preparedToppingDays').value='7';document.getElementById('preparedToppingNote').value='';saveState();renderPreparedToppings();toast('Đã ghi topping');}
function showPreparedToppingEditor(ref){const item=findByRef(state.preparedToppings||[],ref);if(!item||isDeleted(item))return;openModal('Sửa topping',`<div style="display:grid;gap:10px;"><label style="font-size:0.78rem;color:var(--text-muted);">Tên topping</label><input id="editPreparedToppingName" value="${esc(item.name||'')}"><label style="font-size:0.78rem;color:var(--text-muted);">Thời gian nấu</label><input type="datetime-local" id="editPreparedToppingMadeAt" value="${preparedToppingDateTimeInput(item.madeAt)}"><label style="font-size:0.78rem;color:var(--text-muted);">Nhân viên nấu</label><select id="editPreparedToppingStaff">${preparedToppingStaffOptions(item.staffId)}</select><label style="font-size:0.78rem;color:var(--text-muted);">Hạn dùng (ngày)</label><input type="number" min="0.25" step="0.25" id="editPreparedToppingDays" value="${Number(item.shelfLifeDays)||7}"><label style="font-size:0.78rem;color:var(--text-muted);">Ghi chú</label><input id="editPreparedToppingNote" value="${esc(item.note||'')}"><button class="btn btn-primary" onclick="savePreparedToppingEdit('${jsString(ref)}')">Lưu chỉnh sửa</button></div>`);}
function savePreparedToppingEdit(ref){const item=findByRef(state.preparedToppings||[],ref);if(!item||isDeleted(item))return;const before=preparedToppingSnapshotText(item),name=document.getElementById('editPreparedToppingName')?.value.trim(),madeValue=document.getElementById('editPreparedToppingMadeAt')?.value,days=Number(document.getElementById('editPreparedToppingDays')?.value)||7,note=document.getElementById('editPreparedToppingNote')?.value.trim()||'';if(!name){toast('Nhập tên topping');return;}const staff=selectedPreparedToppingStaff('editPreparedToppingStaff'),madeAt=madeValue?new Date(madeValue).getTime():Date.now();item.name=name;item.staffId=staff.id;item.staffName=staff.name;item.madeAt=madeAt;item.madeDate=preparedToppingDateKey(madeAt);item.madeTime=preparedToppingTimeKey(madeAt);item.shelfLifeDays=days;item.expireAt=madeAt+days*PREPARED_TOPPING_DAY_MS;item.note=note;item.status='active';item._lastModified=Date.now();appendPreparedToppingLog(item,'SỬA',before,preparedToppingSnapshotText(item));saveState();renderPreparedToppings();closeModal();toast('Đã sửa topping');}
function deletePreparedTopping(ref){const item=findByRef(state.preparedToppings||[],ref);if(!item||isDeleted(item))return;confirmAction('Xóa record topping này?',()=>{const before=preparedToppingSnapshotText(item);item._deleted=true;item.status='deleted';item._lastModified=Date.now();appendPreparedToppingLog(item,'XÓA',before,'Đã xóa');saveState();renderPreparedToppings();toast('Đã xóa topping');},'Xóa');}
function showPreparedToppingLog(){const logs=(state.editLog||[]).filter(l=>String(l.action||'').startsWith('TOPPING')).sort((a,b)=>itemStamp(b)-itemStamp(a)).slice(0,40);openModal('Lịch sử sửa topping',logs.map(l=>`<div class="edit-log-item"><div><span class="log-time">${esc(l.time||'')}</span> · ${esc(l.staffName||'?')} · ${esc((l.action||'').replace('TOPPING ',''))}</div><div class="log-before">${esc(l.before||'')}</div><div class="log-after">${esc(l.after||'')}</div></div>`).join('')||'<div style="text-align:center;padding:20px;color:var(--text-muted);">Chưa có chỉnh sửa topping</div>');}
function toggleChecklist(t,id){const l=t==='open'?state.openChecklist:state.closeChecklist;const i=activeItems(l).find(c=>c.id===id);if(i){i.checked=!i.checked;i._lastModified=Date.now();}saveState();renderChecklist();}
function renderChecklist(){renderPreparedToppings();['open','close'].forEach(t=>{const l=activeItems(t==='open'?state.openChecklist:state.closeChecklist),c=document.getElementById(t+'Checklist'),p=document.getElementById(t+'Progress');
